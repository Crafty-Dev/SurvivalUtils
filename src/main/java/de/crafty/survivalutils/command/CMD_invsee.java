    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (args[0].equalsIgnoreCase(player.getName())) {
                player.sendMessage(SurvivalUtils.PREFIX + "Why do you wanna look in your own inv? Dumb?");
                return true;
            }
            ...
            SurvivalUtils.OPEN_INVS.add(inv);
            player.openInventory(inv);
            return true;
        }
        player.sendMessage(SurvivalUtils.PREFIX + "\u00a73Could not find player");
        return true;
    }
}
